    ...
    <div className={currentAlbumId ? "h-screen bg-zinc-950 text-white overflow-hidden flex flex-col" : "min-h-screen bg-zinc-950 text-white"}>
      ...
      <div className={currentAlbumId ? "flex-1 min-h-0 max-w-7xl w-full mx-auto px-6 flex gap-6" : ""}>
        ...
        {nowPlaying && (
          ...
          >
            {/* Mobile: album art + Open in Spotify button */}
            <div className={currentAlbumId ? "flex flex-col items-center gap-6 py-4 lg:hidden" : "hidden"}>
              <img
                src={nowPlaying.coverUrl}
                alt={`${nowPlaying.title} by ${nowPlaying.artist}`}
                className="w-3/4 max-w-xs rounded-xl shadow-lg"
              />
              <div className="text-center">
                <h1 className="text-2xl font-extrabold text-white">{nowPlaying.title}</h1>
              ...
              <a
                ...
                className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-[#1DB954] text-white font-bold text-lg hover:bg-[#1ed760] transition-colors"
              >
            ...
            <div className="rounded-xl overflow-hidden flex-1 min-h-0 hidden lg:block">
              <iframe
